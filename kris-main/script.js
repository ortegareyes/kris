...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function applyConfig() {
        if (window.CONFIG) {
            ...
            if (specialMessageTitle) {
                specialMessageTitle.textContent = `Para mi querida ${CONFIG.partnerName}`;
            }
    ...
    // randomly float images on top of the pink background
    function createFloatingImage() {
        if (window.CONFIG && Array.isArray(CONFIG.floatingImages) && CONFIG.floatingImages.length > 0) {
            const heartsContainer = document.querySelector('.hearts-container');
            if (!heartsContainer) return;
            const img = document.createElement('img');
            img.src = CONFIG.floatingImages[Math.floor(Math.random() * CONFIG.floatingImages.length)];
    ...
    let isPlaying = false;
    // allow custom file from config
    if (window.CONFIG && CONFIG.musicFile) {
        // replace inner HTML sources
        bgMusic.innerHTML = `<source src="${CONFIG.musicFile}" type="audio/mp3">`;
        // reload/initialize
        bgMusic.load();
    }

    // new amorcitokris playback elements
    const amorToggle = document.getElementById('amorToggle');
    const amorMusic = document.getElementById('amorMusic');
    let amorPlaying = false;
    ...
    // listener for amorcitokris track
    amorToggle.addEventListener('click', function() {
        if (amorPlaying) {
            amorMusic.pause();
            amorToggle.textContent = '🎶';
            amorPlaying = false;
        } else {
            amorMusic.play().then(() => {
                amorToggle.textContent = '🔊';
                ...
                amorToggle.textContent = '❌';
                console.log('Amor music could not be played');
            });
        }
    });
